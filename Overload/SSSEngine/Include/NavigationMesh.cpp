...
void CNavigationMesh::AddCell(const Vector3 vPos[3])
{
	...
	Vector3	vDir1 = vPos[1] - vPos[0];
	Vector3	vDir2 = vPos[2] - vPos[0];

	vDir1 = vDir1.Normalize();
	vDir2 = vDir2.Normalize();

	Vector3	vFaceNormal = vDir1.Cross(vDir2);
	Vector3	vView = Vector3::Axis[AX_Y];

	float	fAngle = vView.Angle(vFaceNormal) * 180.f / PI;
...
void CNavigationMesh::FindPath(const Vector3 & vStart, const Vector3 & vEnd)
{
	...
	for (size_t i = 0; i < m_vecCell.size(); ++i)
	{
		...
		m_vecCell[i]->m_eType = NCLT_NONE;
	}

	// 시작 Cell을 Open목록에 넣어준다.
	pStartCell->m_eType = NCLT_OPEN;
	pStartCell->m_fG = 0.f; 
	pStartCell->m_fH = vStart.Length();//vStart.Distance(*this, vEnd);
	...
	const Vector3& vEnd, const Vector3& vStart)
{
	for (size_t i = 0; i < pCell->m_vecAdj.size(); ++i)
	{
		...
		if (!pAdjCell->m_bEnable)
			continue;

		if (pAdjCell->m_eType != NCLT_CLOSE)
		{
...
float CNavigationMesh::GetY(const Vector3 & vPos)
{
	...
	Vector3	vCellSize = (m_vMax - m_vMin) / m_iRectCount;
	...
	int	idxZ = m_iRectCount - ((int)vConvertPos.z + 1);

	if (idxX < 0 || idxX > m_iRectCount)
		return 0.f;

	else if (idxZ < 0 || idxZ > m_iRectCount)
		return 0.f;

	int	idx = (idxZ * m_iRectCount + idxX) * 2;

	if (!m_vecCell[idx]->m_bEnable)
		return 0.f;

	Vector3	vLTPos = m_vecCell[idx]->m_vPos[0];
	vLTPos.x /= vCellSize.x;
	vLTPos.z /= vCellSize.z;


	Vector3	vConvertPos1 = vLTPos - m_vMin;

	float	fX = vConvertPos.x - vConvertPos1.x;
	float	fZ = vConvertPos1.z - vConvertPos.z;

	// 우상단 삼각형
	if (fX >= fZ)
	{
		CNavigationCell*	pCell = m_vecCell[idx];
		float	fY[3];
		for (int i = 0; i < 3; ++i)
		{
			fY[i] = pCell->m_vPos[i].y;
		}
		return fY[0] + (fY[1] - fY[0]) * fX + (fY[2] - fY[1]) * fZ;
	}

	// 좌하단 삼각형
	CNavigationCell*	pCell = m_vecCell[idx + 1];
	float	fY[3];
	for (int i = 0; i < 3; ++i)
	{
		fY[i] = pCell->m_vPos[i].y;
	...
	return fY[0] + (fY[2] - fY[1]) * fX + (fY[1] - fY[0]) * fZ;
}
